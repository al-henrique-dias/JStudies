...
let price = 10.99;
let online = true
console.log(`Your name is ${fullName}\nname is a`, typeof fullName);//to format console text and display in-conde values, use `${}` (ps: do not use "" or '')
console.log(`You are ${age} years old\nage is a`, typeof age);//"typeof" reserved keyword returns the datatype of a variable in a string
...
pintado.mover();
pintado.mover(pintado.veloNado);//the parameter has been ignored
//getters (method to get a property values) and setters (method to give values to a property)
class Rectangle{
    ...
    set width(newWidth){//the "set" reserved keyword indicates this is the declaration of a method which set as value to the attribute with the same name of the method
        if(newWidth > 0)//setters are great for input validation
            this._width = newWidth;//the underscore "_" before the attribute indicates that the attribute is private (can only be accessed inside this class)
        else
            console.error("Width must be a positive number");//console.error() displays a error message at the console
    }
    ...
    get width(){//the "get" reserved keyword indicates this is the declaration of a method which returns the value of the attribute with the same name of the method
        return this._width;
    }
    get height(){
        return this._height;
    }
    get area(){//this can even create attributes that have not been defined at the constructor
        return `${(this._width * this._height).toFixed(1)} cm²`; //you can also format the returned value, but notice that it returns a string so watchout for the possible need for content trim and datatype conversion
    }
}

const rectangle = new Rectangle(3, 4);
rectangle.width = 5;//this call the setter
rectangle.height = 6;
rectangle.width = -10;//this is blocked by the setter
rectangle.height = "pizza"//this is blocked by the setter
console.log(rectangle.width, rectangle.height, rectangle.area);//this calls the getter
//rectangle.area is not a proper attribute, it has been generated by it's getter
...
class Person{
    ...
    get userName(){
        return this._firstName+'_'+this.lastName;
    }
    get age(){
        return this._age;
    }
}
const personA = new Person("Spongebob", "Squarepantas", 30);
console.log(personA.userName);
console.log(personA.firstName, personA.lastName, personA.age);
...
[a, b] = [b, a];//swapping the value of two variables

console.log(a, b);//values swapped (fullfilling the prophesy)

const colors = ["red", "green", "blue", "black", "white"];

[colors[0], colors[4]] = [colors[4], colors[0]];//swapping the value of two arrays

console.log(colors);//values swapped (prophesy fullfilled again)

const [firstColor, secondColor, thirdColor, ...extraColors] = colors;//assign array elements to variables and other arrays
...
const {firstName, lastName, years, job="unemployed"} = personBeta;//extract values from objects
                                    //values extracted can even have default values
console.log(firstName, lastName, years, job);

function displayPerson({firstName, lastName, years, job="unemployed"}){//distructure in function parameters
                                                    //again we can have default values
    console.log(`name: ${firstName} ${lastName}`);
    console.log(`age: ${years}`);
    console.log(`job: ${job}`);
}

displayPerson(personAlpha);
displayPerson(personBeta);

/**
 * nested objects - objects inside of objects
 * allows you to represent more complex data structures
 * child object is an object enclosed by another object (parent object)
 */
const pessoa = {
    nome: "Bob Esponja",
    idade: 30,
    estudante: true,
    hobbies: ["karatê", "caçar água-viva", "cozinhar"],
    endereco: {//nested object
        rua: "Rua Concha, 123",
        cidade: "Fenda do Bikini",
        territorio: "Águas internacionais"
    }
}

console.log(pessoa.nome, pessoa.idade, pessoa.estudante, pessoa.hobbies[2], pessoa.endereco.rua);
...
class Personage{
    constructor(name, age, ...address){
        this.name = name;
        this.age = age;
        this.address = new Address(...address);//you can nest class objects
    }
}
class Address{
...
const personage1 = new Personage("Spongebob", 30, "124 Conch St",
                                              "Bikini Bottom",
                                              "Int Waters");
const personage2 = new Personage("Patrick", 34, "122 Conch St",
                                              "Bikini Bottom",
                                              "Int Waters");
const personage3 = new Personage("Squidward", 45, "123 Conch St",
                                              "Bikini Bottom",
                                              "Int Waters");
console.log(personage1.address.street);
console.log(personage2.address.street);
console.log(personage3.address.street);

const frutas = [
    {nome: "maçã", cor: "vermelho", calorias: 95},
    {nome: "laranja", cor: "laranja", calorias: 45},
    {nome: "banana", cor: "amarelo", calorias: 105},
    {nome: "maracuja", cor: "amarelo", calorias: 37},
    {nome: "abacaxi", cor: "amarelo", calorias: 100},
    
];
console.log(frutas[4].calorias);
frutas.splice(1, 1);
frutas.pop();
frutas.push({nome: "uva", cor: "roxo", calorias: 62});
frutas.forEach(fruta => console.log(fruta));
const nomesFrutas = frutas.map(fruta => fruta.nome);
console.log(nomesFrutas);
const frutasAmarelas = frutas.filter(fruta => fruta.cor === "amarelo");
console.log(frutasAmarelas);
const aFrutaGorda = frutas.reduce( (frutaGorda, frutaComparada) => //fruta com mais calorias
                                        frutaComparada.calorias > frutaGorda.calorias ?
                                            frutaComparada
                                        : frutaGorda);
const aFrutaMagra = frutas.reduce( (frutaMagra, frutaComparada) => //fruta com menos calorias
                                        frutaComparada.calorias < frutaMagra.calorias ?
                                            frutaComparada
                                        : frutaMagra);
console.log(aFrutaGorda);
console.log(aFrutaMagra);